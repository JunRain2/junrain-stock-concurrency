...
import com.junrain.stock.member.domain.Member
import com.junrain.stock.member.domain.MemberRepository
import com.junrain.stock.member.domain.MemberType
...
class JdbcBulkInsertProductRepositoryTest {
    ...
    @Test
    fun `모든 제품이 성공적으로 저장되면 ProductCode를 반환해야 한다`() =
        runBlocking {
            // given
            val products =
                listOf(
    ...
    @Test
    fun `중복된 코드가 있어도 모든 제품의 ProductCode를 반환해야 한다`() =
        runBlocking {
            // given
    ...
    @Test
    fun `모든 제품이 중복되어도 ProductCode를 반환해야 한다`() =
        runBlocking {
            // given
    ...
    @Test
    fun `단일 제품이 성공적으로 저장되면 ProductCode를 반환해야 한다`() =
        runBlocking {
            // given
    ...
    @Test
    fun `단일 제품이 중복되어도 ProductCode를 반환해야 한다`() =
        runBlocking {
            // given
    ...
    @Test
    fun `청크 크기를 초과하는 제품 리스트를 저장할 수 있어야 한다`() =
        runBlocking {
            // given
            val largeProductList =
                (1..chunkSize + 10).map { i ->
    ...
    @Test
    fun `청크 크기를 정확히 맞추는 제품 리스트를 저장할 수 있어야 한다`() =
        runBlocking {
            // given
            val products =
                (1..chunkSize).map { i ->
    ...
    @Test
    fun `대량의 제품 중 일부가 중복되어도 모든 ProductCode를 반환해야 한다`() =
        runBlocking {
            // given
            val existingProducts =
                (1..5).map { i ->
