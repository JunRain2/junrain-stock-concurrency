...
class BulkInsertProductServiceIntegrationTest {
    ...
    fun `should handle duplicate product codes with INSERT IGNORE`() {
        ...
        bulkInsertProductService.registerProducts(firstBatch)

        // when: 동일한 코드를 포함한 10개 상품 재등록 시도 (처음 5개는 중복)
        val secondBatch = createValidProducts(10)
        val result = bulkInsertProductService.registerProducts(secondBatch)
    ...
    fun `should not save products that fail business validation`() {
        ...
        assertTrue(result.failedProducts.any { it.message == "상품명은 20자 이하여야 합니다" })
    ...
    fun `should handle mixed scenario with validation failures, duplicates, and successes`() {
        ...
        bulkInsertProductService.registerProducts(existingProducts)
        ...
        assertTrue(result.failedProducts.any { it.message == "상품명은 필수입니다" })
        assertTrue(result.failedProducts.any { it.message == "데이터를 삽입하던 도중 문제가 발생했습니다." })
