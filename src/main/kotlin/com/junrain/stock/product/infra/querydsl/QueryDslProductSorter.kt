package com.junrain.stock.product.infra.querydsl

import com.junrain.stock.product.application.query.ProductSorter
import com.junrain.stock.product.domain.QProduct
...
sealed class QueryDslProductSorter {
    ...
    class LatestSorter(
        ...
        override fun buildSortOrder(qProduct: QProduct): List<OrderSpecifier<*>> = listOf(qProduct.createdAt.desc())

        override fun hasCursor(): Boolean = lastProductId != null && createdAt != null

        override fun buildCondition(qProduct: QProduct): Predicate =
            qProduct.createdAt.lt(this.createdAt).or(
                qProduct.createdAt
                    .eq(this.createdAt)
    ...
    class SalePriceAsc(
        ...
        override fun buildSortOrder(qProduct: QProduct): List<OrderSpecifier<*>> = listOf(qProduct.price.amount.asc())

        override fun hasCursor(): Boolean = lastProductId != null && price != null

        override fun buildCondition(qProduct: QProduct): Predicate =
            qProduct.price.amount.gt(this.price).or(
    ...
    class SalePriceDesc(
        ...
        override fun buildSortOrder(qProduct: QProduct): List<OrderSpecifier<*>> = listOf(qProduct.price.amount.desc())

        override fun hasCursor(): Boolean = lastProductId != null && price != null

        override fun buildCondition(qProduct: QProduct): Predicate =
            qProduct.price.amount.lt(this.price).or(
